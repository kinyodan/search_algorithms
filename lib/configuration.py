...
def read_config(config_file: str) -> Dict[str, Any]:
    ...
    for section in config.sections():
        settings['file_path'] = config.get(
            section, 'linuxpath', fallback=settings['file_path']
        )
        settings['use_ssl'] = config.getboolean(
            section, 'use_ssl', fallback=settings['use_ssl']
        )
        settings['ssl_certfile'] = config.get(
            section, 'ssl_certfile', fallback=settings['ssl_certfile']
        )
        settings['ssl_keyfile'] = config.get(
            section, 'ssl_keyfile', fallback=settings['ssl_keyfile']
        )
        settings['ssl_psk_keyfile'] = config.get(
            section, 'ssl_psk_keyfile', fallback=settings['ssl_psk_keyfile']
        )
        settings['reread_on_query_config'] = config.get(
            section, 'reread_on_query_config',
            fallback=settings['reread_on_query_config']
        )
        settings['metrics_path'] = config.get(
            section, 'metrics_path', fallback=settings['metrics_path']
        )
        settings['algorithms_list'] = config.get(
            section, 'algorithms_list', fallback=settings['algorithms_list']
        )

    return settings


def read_client_config(config_file: str) -> Dict[str, Any]:
    """Read the configuration file and fetch settings.
...
def load_reread_on_query_config(
        config_file_path: str, data_file_path: str) -> bool:
    """
    Load or update the 'reread_on_query'
    configuration from a specified config file.
