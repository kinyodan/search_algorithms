...
def update_metrics(index: int, exec_times: list, reread_on_query: bool = False):
    ...
    print(f"DEBUG: Execution times {exec_times} added to '{key}' at index {index}.")


def run_tests(file_size: int, index: int, num_tests: int = 5, reread_on_query: bool = False):
    ...
    Args:
        ...
        index (int): Index corresponding to the file size in the JSON structure.
        num_tests (int): Number of test runs to average results.
        reread_on_query (bool): Whether the server is rereading the file on each query.
    """
    host = 'localhost'
    port = 44445
    query = {
    ...
    if exec_times:
        ...
        print(f"DEBUG: No valid execution times recorded for file size {file_size}.")
