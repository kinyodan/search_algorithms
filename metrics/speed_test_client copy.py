...
def test_file_size(file_size: int, num_queries: int) -> float:
    ...
    query = {"query_string": file_content.splitlines()[0]}  # Send the first line as query

    exec_times = []
    
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(send_query, host, port, query) for _ in range(num_queries)]
...
if __name__ == "__main__":
    ...
    file_size = 100000  # For this example, testing with a file size of 10,000
    performance_data = performance_test(file_size)
    plot_stacked_line_chart(performance_data)
